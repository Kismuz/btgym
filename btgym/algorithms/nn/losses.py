...
def aac_loss_def(act_target, adv_target, r_target, pi_logits, pi_vf, pi_prime_logits,
    ...
    """
    with tf.compat.v1.name_scope(name + '/aac'):
        neg_pi_log_prob = tf.nn.softmax_cross_entropy_with_logits(
            logits=pi_logits,
            labels=act_target
        )
        pi_loss = tf.reduce_mean(input_tensor=neg_pi_log_prob * adv_target)
        vf_loss = 0.5 * tf.compat.v1.losses.mean_squared_error(r_target, pi_vf)
        entropy = tf.reduce_mean(input_tensor=cat_entropy(pi_logits))

        loss = pi_loss + vf_loss - entropy * entropy_beta

        mean_vf = tf.reduce_mean(input_tensor=pi_vf)
        mean_t_target = tf.reduce_mean(input_tensor=r_target)

        summaries = [
            tf.compat.v1.summary.scalar('policy_loss', pi_loss),
            tf.compat.v1.summary.scalar('value_loss', vf_loss),
        ]
        if verbose:
            summaries += [
                tf.compat.v1.summary.scalar('entropy', entropy),
                tf.compat.v1.summary.scalar('value_fn', mean_vf),
...
def ppo_loss_def(act_target, adv_target, r_target, pi_logits, pi_vf, pi_prime_logits, entropy_beta, epsilon,
    ...
    with tf.compat.v1.name_scope(name + '/ppo'):
        ...
        pi_surr_loss = - tf.reduce_mean(input_tensor=tf.minimum(surr1, surr2))  # PPO's pessimistic surrogate (L^CLIP)
        vf_loss = tf.compat.v1.losses.mean_squared_error(r_target, pi_vf)  # V.fn. loss
        entropy = tf.reduce_mean(input_tensor=cat_entropy(pi_logits))

        loss = pi_surr_loss + vf_loss - entropy * entropy_beta

        # Info:
        mean_pi_ratio = tf.reduce_mean(input_tensor=pi_ratio)
        mean_vf = tf.reduce_mean(input_tensor=pi_vf)
        mean_kl_old_new = tf.reduce_mean(input_tensor=kl_divergence(pi_prime_logits, pi_logits))

        summaries = [
            tf.compat.v1.summary.scalar('l_clip_loss', pi_surr_loss),
            tf.compat.v1.summary.scalar('value_loss', vf_loss),
        ]
        if verbose:
            summaries += [
                tf.compat.v1.summary.scalar('entropy', entropy),
                tf.compat.v1.summary.scalar('Dkl_old_new', mean_kl_old_new),
                tf.compat.v1.summary.scalar('pi_ratio', mean_pi_ratio),
                tf.compat.v1.summary.scalar('value_fn', mean_vf),
            ]
...
def value_fn_loss_def(r_target, pi_vf, name='_vr_', verbose=False):
    ...
    with tf.compat.v1.name_scope(name + '/value_replay'):
        loss = tf.compat.v1.losses.mean_squared_error(r_target, pi_vf)

        if verbose:
            summaries = [tf.compat.v1.summary.scalar('v_loss', loss)]
        else:
            summaries = []
...
def pc_loss_def(actions, targets, pi_pc_q, name='_pc_', verbose=False):
    ...
    with tf.compat.v1.name_scope(name + '/pixel_control'):
        # Get Q-value features for actions been taken and define loss:
        pc_action_reshaped = tf.reshape(actions, [-1, 1, 1, tf.shape(input=actions)[-1]])
        pc_q_action = tf.multiply(pi_pc_q, pc_action_reshaped)
        pc_q_action = tf.reduce_sum(input_tensor=pc_q_action, axis=-1, keepdims=False)

        batch_size = tf.shape(input=targets)[0]
        loss = tf.reduce_sum(input_tensor=tf.square(targets - pc_q_action)) / tf.cast(batch_size, tf.float32)
...
def rp_loss_def(rp_targets, pi_rp_logits, name='_rp_', verbose=False):
    ...
    with tf.compat.v1.name_scope(name + '/reward_prediction'):
        loss = tf.nn.softmax_cross_entropy_with_logits(
            labels=rp_targets,
            logits=pi_rp_logits
        )[0]
        if verbose:
            summaries = [tf.compat.v1.summary.scalar('class_loss', loss), ]
        else:
            summaries = []
...
def ae_loss_def(targets, logits, alpha=1.0, name='ae_loss', verbose=False, **kwargs):
    ...
    Returns:
        ...
        list of summarues
    """
    with tf.compat.v1.name_scope(name + '/ae'):
        loss = tf.compat.v1.losses.mean_squared_error(targets, logits)

        if verbose:
            summaries = [tf.compat.v1.summary.scalar('reconstruct_loss', loss)]
        else:
            summaries = []
...
def beta_vae_loss_def(targets, logits, d_kl, alpha=1.0, beta=1.0, name='beta_vae_loss', verbose=False):
    ...
    """
    with tf.compat.v1.name_scope(name + '/b_vae'):
        r_loss = tf.compat.v1.losses.mean_squared_error(targets, logits)
        vae_loss = tf.reduce_mean(input_tensor=d_kl)
        loss = alpha * r_loss + beta * vae_loss
        if verbose:
            summaries = [
                tf.compat.v1.summary.scalar('reconstruct_loss', r_loss),
                tf.compat.v1.summary.scalar('d_kl_loss', vae_loss),
            ]
        else:
            summaries = []
